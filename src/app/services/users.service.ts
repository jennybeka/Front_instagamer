...
export class UsersService {
  ...
  createPost(image_url: string, text_photo: string, tags_image: string, legend: string): Observable<any> {
    const token = sessionStorage.getItem('token');
    const headers = new HttpHeaders({
      Authorization: 'Bearer ' + token
    });
    console.log(`Chamando o outro ${tags_image}`)
    return this.http.post('http://localhost:3001/instagamer/posts/create', {image_url, text_photo, tags_image, legend}, { headers } );
  }
